...
var VIEWDATA={
    ...
    init_event: function(){
        ...
        $("#add-pay-record").click(function(){
            var fundid = $("#fundid").val();
            var projectid = $("#projectid").val();
            var paydate = $("#paydate").val();
            var paytotal = MONEYFORMAT.toNumber($("#paytotal").val());

            var paytype = $('input[name=optionsRadios]:checked').val();


            var model = {
                payDate:DATEFORMAT.toRest(paydate),
                amount:paytotal,
                payType:paytype,
                project:projectid,
                fund:fundid
            };
            me.add_pay_record(model);
        });

    },
    add_pay_record: function(model){
        console.log("model:",JSON.stringify(model));

        var me = this;
        var data = {url: '/api/payRecord/create', entity: JSON.stringify(model)};
        console.log(data);
        var me = this;
        $.ajax({
            type: 'post',
